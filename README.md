...
Todo lo de esta sección es **opcional**. Una llamada existente —
`sendInvoice($invoice, $previous)` o `sendInvoice($invoice, $previous, $record)` —
no cambia ni un byte del XML que envía.
...
$result = $aeatClient->sendInvoice(
    ...
    [                          // $record: la huella recalculada y almacenada
        'hash'         => $invoice->hash,
        'generated_at' => $invoice->hash_generated_at,
    ],
...
huella por su cuenta si se le pasa una en `$record`: la que se almacena y la que
se envía tienen que ser la misma o la cadena se bifurca.
...
$result = $aeatClient->sendCancellation(
    ...
    $previous,
    [
        'hash'         => $invoice->cancel_hash,
        'generated_at' => $invoice->cancel_generated_at,
    ],
